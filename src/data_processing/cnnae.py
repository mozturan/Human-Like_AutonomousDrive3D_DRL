...
class ConvolutionalAutoencoder:
    ...
    def build_autoencoder(self):
        ...
        x = MaxPool2D(pool_size=2, strides=1, name= "MaxPool2D")(x)
        x = Flatten(name = "Flatten")(x)

        # Decoder
        x = Dense(2048, activation='relu')(x)
        x = Reshape((7,7,64))(x)
        x = Conv2DTranspose(64, 3, strides=1, activation='relu', padding='same')(x)
        x = Conv2DTranspose(64, 3, strides=2, activation='relu', padding='same')(x)
        x = Conv2DTranspose(32, 4, strides=2, activation='relu', padding='same')(x)
        x = Conv2DTranspose(3, 8, strides=4, activation='sigmoid', padding='same')(x)
        decoded = x
        # # Decoder 
